...
class SizeLabel:
    ...
    def _draw_text(self):
        ...
        if self.type in ['width', 'dlo_width']:
            ...
            if self.type == 'width':
                self.context.show_text(f"{self.parent_panel.name.upper()}: {self.__convert_to_fraction(self.parent_panel.width)}'")
            elif self.type == 'dlo_width':
                self.context.show_text(f"{self.parent_panel.name.upper()} DLO: {self.parent_panel.dlo_width}'")
            ...
            if self.type == 'height':
                self.context.show_text(f"{self.parent_panel.name.upper()}: {self.parent_panel.height}'")
            elif self.type == 'dlo_height':
                self.context.show_text(f"{self.parent_panel.name.upper()} DLO: {self.parent_panel.dlo_height}'")
    ...
    def x2(self):
        if self.type in ['width', 'dlo_width']:
            ...
            if vertical_labels:
                min_x_point = min([min(_.x2, _.x3) for _ in vertical_labels])
            else:
...
class Panel:
    ...
    def __init__(self, width, height, panel_type, coordinates, dlo_width=0, dlo_height=0, x=0, y=0, parent_panel=None,
                 child_frames=(),
                 child_panels=(), move_direction=None, panel_name='FRAME', raw_params=None):
        self._context = None

        self.width = width
        self.height = height
        self.dlo_width = dlo_width
        self.dlo_height = dlo_height
        self.x = x
        self.y = y
    ...
    def _draw_child_panels(self):
        ...
        for child_panel in self.raw_params.get('panels', []):
            ...
            panel = Panel(
                ...
                coordinates=child_panel['coordinates'],
                x=self.x + x_offset,
                y=self.y + y_offset,
                parent_panel=self,
                child_frames=child_panel.get('frames') or [],
                child_panels=child_panel.get('panels') or [],
    ...
    def draw(self):
        ...
        #     _coords_x = child_frame['coordinates']['x']
        #     _coords_y = child_frame['coordinates']['y']
        #
        #     # if _coords_x > 1:
        #     #     (_ for _ in self.parent_panel.child_frames)
        #     #
        #     # if _coords_y > 1:
        #     #     pass
        #
        ...
        #         panel_type='frame',
        #         coordinates=child_frame['coordinates'],
        #         x=self.x,
        #         y=self.y,
        #         parent_panel=self,
        #         child_frames=child_frame.get('frames') or [],
        #         child_panels=child_frame.get('panels') or []
        #     ).set_context(self.context)
        #
        #     panel.draw()
        #     panel.draw_width_size()
        #     panel.draw_height_size()
    ...
    def size_labels(self):
        # child_frames_labels = list(itertools.chain(*[_._size_labels for _ in self.child_frames]))
